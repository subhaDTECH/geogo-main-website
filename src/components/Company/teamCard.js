                    ...
                    {teams &&
                      teams?.map((team, index) => {
                        return (
                          ...
                          <div
                            key={index}
                            className="group h-[300px] relative overflow-hidden rounded-md cursor-pointer"
                          >
                            <img
                              src={team?.image?.localFile?.url}
                              alt={`${team?.name}'s Photo`}
                              className="w-full h-full object-cover"
                            />
                            <div className="group-hover:h-[120px] transition-height duration-300 absolute bottom-0 left-0 right-0 p-4 bg-black bg-opacity-50 text-white h-[80px] transition-height duration-300">
                              <p className="text-lg font-bold">{team?.name}</p>
                              <p className="text-sm">{team?.position_type}</p>
                            </div>
                          </div>
                        )
                      })}
